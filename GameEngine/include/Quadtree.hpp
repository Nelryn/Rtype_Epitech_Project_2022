...
namespace sfge {
	class Quadtree {
		...
		/**
			...
			* @param  marge: Marge for collision
			* @retval None
			*/
			Quadtree(float x, float y, float width, float height, int level, int maxLevel, float marge);
			...
			* @retval Void
			*/
			void AddObject(std::vector<std::shared_ptr<Entity>> objects);
			/**
			...
			* @retval bool
			*/
			bool contains(Quadtree *child, std::shared_ptr<Entity> object);
			/**
			...
			* @retval Void
			*/
			std::vector< unsigned int> detectCollision();
		protected:
		private:
			/**
