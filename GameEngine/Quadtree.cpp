...
namespace sfge {
	Quadtree::Quadtree(float x, float y, float width, float height, int level, int maxLevel, float marge) : _x(x), _y(y), _width(width), _height(height), _level(level), _maxLevel(maxLevel), _marge(marge)
	{
		...
		_NW = new Quadtree(_x, _y, _width / 2.0f, _height / 2.0f, _level+1, _maxLevel, _marge);
		_NE = new Quadtree(_x + _width / 2.0f, _y, _width / 2.0f, _height / 2.0f, _level+1, _maxLevel, _marge);
		_SW = new Quadtree(_x, _y + _height / 2.0f, _width / 2.0f, _height / 2.0f, _level+1, _maxLevel, _marge);
		_SE = new Quadtree(_x + _width / 2.0f, _y + _height / 2.0f, _width / 2.0f, _height / 2.0f, _level+1, _maxLevel, _marge);
	...
	std::vector<unsigned int> Quadtree::detectCollision()
	{
		if (_level == _maxLevel) {
			for (auto i = _objects.begin(); i != _objects.end(); i++) {
				for (auto y = _objects.begin(); y != _objects.end(); y++) {
					if ((*i) != (*y)) {
						if ((*i)->pos.x - _marge < (*y)->pos.x + (*y)->size.x
						&& (*i)->pos.x + (*i)->size.x + _marge > (*y)->pos.x
						&& (*i)->pos.y - _marge < (*y)->pos.y + (*y)->size.y
						&& (*i)->pos.y + (*i)->size.y + _marge > (*y)->pos.y) {
							//std::cout << "Collision detected for ids: " << (*i)->getId() << " and " << (*y)->getId() << std::endl;
							return (std::vector<unsigned int> {(*i)->getId(), (*y)->getId()});
						}
					}
				}
			}
		...
		ids = _NE->detectCollision();

		std::vector<unsigned int> ids1 = _NW->detectCollision();
		std::vector<unsigned int> ids2 = _SE->detectCollision();
		std::vector<unsigned int> ids3 = _SW->detectCollision();



		ids.insert(ids.end(), ids1.begin(), ids1.end());
